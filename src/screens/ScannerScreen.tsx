...
const ScannerScreen = () => {
  const [cameraRef, setCameraRef] = useRef<any>(null);
  const [isProcessing, setIsProcessing] = useState(false);
  const [capturedImage, setCapturedImage] = useState<string | null>(null);
  const [extractedText, setExtractedText] = useState<string>('');
  const [contactInfo, setContactInfo] = useState<ContactInfo>({});
  const [showResults, setShowResults] = useState(false);
  const [permissionStatus, setPermissionStatus] = useState<'undetermined' | 'denied' | 'authorized'>('undetermined');
  ...
  const requestCameraPermission = useCallback(async () => {
    try {
      ...
      setPermissionStatus(status as 'undetermined' | 'denied' | 'authorized');
  ...
  const parseContactInfo = (text: string): ContactInfo => {
    ...
    const companyMatch = text.match(/(?:Inc|LLC|Ltd|Corp|Corporation|Company|Co\.)/i);
    if (companyMatch) {
      ...
      {!showResults ? (
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.overlay}>
            ...
            <Text style={styles.instructionText}>Point camera at business card and tap to capture</Text>
          </View>
          <TouchableOpacity 
        ...
        <View style={Styles.resultsContainer}>
          ...
          <View style={Styles.contactInfoContainer}>
            ...
            <Text style={Styles.contactInfoValue}>{contactInfo.name || 'Not detected'}</Text>
            
            <Text style={Styles.contactInfoLabel}>Email:</Text>
            <Text style={Styles.contactInfoValue}>{contactInfo.email || 'Not detected'}</Text>
            
            <Text style={Styles.contactInfoLabel}>Phone:</Text>
            <Text style={Styles.contactInfoValue}>{contactInfo.phone || 'Not detected'}</Text>
            
            <Text style={Styles.contactInfoLabel}>Company:</Text>
            <Text style={Styles.contactInfoValue}>{contactInfo.company || 'Not detected'}</Text>
            
            <Text style={Styles.contactInfoLabel}>Website:</Text>
            <Text style={Styles.contactInfoValue}>{contactInfo.website || 'Not detected'}</Text>
          </View>
          ...
          <View style={Styles.buttonContainer}>
            ...
            <TouchableOpacity style={Styles.button} onPress={handleExportContact}>
